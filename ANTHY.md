...
- ~~BIT-10 (Novedades del Establecimiento)~~ — **cerrado el 10 Ago 2026.** BIT reutilizada (era "formulario de ficha clínica"). Diseño funcional completo (Tipo D) + implementación ejecutada en la misma BIT. Módulo completamente autónomo: 4 archivos en `rodeo/js/modules/novedades/`, entrada "📰 Novedades" en sidebar (entre Visitas y Observaciones), rutas registradas. Funcionalidades: listado con Fecha/Tipo/Descripción/Paciente-Lote; alta; edición; campo `precipitacion_mm` condicional (visible solo cuando tipo = Clima); soft delete exclusivo para ADMINISTRADOR. Migración ejecutada en producción: tablas `novedades_establecimiento` + `novedades_adjuntos`, trigger `novedades_updated_by`, función `get_mi_rol()` (SECURITY DEFINER), 7 políticas RLS. Corrección clave de implementación: `WITH CHECK` explícito con dos ramas en `novedades_update` para permitir soft delete sin bloqueo por PostgreSQL. Validado por Claudy (Supabase SQL Editor) + validación visual manual por Brenda en producción. Playwright no ejecutado (limitación de entorno). Posición de Novedades en Informe Diario (después de VISITAS) pendiente de confirmación con Etel antes de BIT-12.
