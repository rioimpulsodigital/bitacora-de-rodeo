...
- ~~BIT-10 (Novedades del Establecimiento)~~ — **cerrado el 10 Ago 2026.** BIT reutilizada (era "formulario de ficha clínica"). Diseño funcional completo (Tipo D) + implementación ejecutada en la misma BIT. Módulo completamente autónomo: 4 archivos en `rodeo/js/modules/novedades/`, entrada "📰 Novedades" en sidebar (entre Visitas y Observaciones), rutas registradas. Funcionalidades entregadas: listado con Fecha/Tipo/Descripción/Paciente-Lote; alta; edición; campo `precipitacion_mm` condicional (visible solo cuando tipo = Clima). Migración ejecutada en producción: tablas `novedades_establecimiento` + `novedades_adjuntos`, trigger `novedades_updated_by`, función `get_mi_rol()` + función `soft_delete_novedad` (SECURITY DEFINER), 7 políticas RLS. Validado por Claudy (Supabase SQL Editor) + validación visual manual por Brenda en producción. Playwright no ejecutado (limitación de entorno). **Eliminación diferida a BIT-35**: el botón "Eliminar" fue ocultado de la UI; la implementación técnica subyacente (RPC `soft_delete_novedad`, policy `novedades_delete`) se conserva como base para BIT-35. Posición de Novedades en Informe Diario (después de VISITAS) pendiente de confirmación con Etel antes de BIT-12.
...
- **BIT-11 (Atenciones Clínicas) — 🔄 En curso (13 Ago 2026):** Frontend completo (4 archivos: `services.js`, `list.js`, `form.js`, `index.js` + `migration-BIT-11.sql`). Modelo de permisos V1 definitivo: rol único por usuario, PROFESIONAL crea/edita-propias, ADMINISTRADOR ve-todas/edita-todas/no-crea, OPERADOR_CAMPO sin acceso. `profesional_responsable_id` siempre lectura (auto-completado en creación, display en edición). **Pendiente de Claudy:** ejecutar queries de verificación → ejecutar `migration-BIT-11.sql` → validación manual de los 7 escenarios documentados. Alertas de próxima visita: implementadas como banner in-app (suplementario); alertas por email pendientes de infraestructura (BIT futuro). No commitear hasta que Brenda apruebe todas las validaciones. No cerrar BIT-11 en Notion sin validación completa.
- **BIT-34 (Gastos del Establecimiento) — ⏳ pendiente:** relevamiento y diseño. No usar este BIT para implementar multi-rol ni cambios de arquitectura de permisos — esa confusión existió en sesiones anteriores y está resuelta. Alcance: módulo de registro de gastos operativos del establecimiento.
- **Roadmap reorganizado (BIT-32 auditoría + BIT-33 roadmap, 30 Jul 2026):** Ver BIT-33 en Notion para el plan maestro completo. ~~BIT-10~~ cerrado el 10 Ago 2026. BIT-11 en curso (ver arriba). Siguientes: BIT-12 (Informe Diario), BIT-13 (Fotografías y adjuntos transversales — Paciente, Novedades, Atenciones), BIT-14 (Paciente Animal: Especie/Categoría/Sexo), BIT-34 (Gastos del Establecimiento — no usar para roles), BIT-35 (eliminación/papelera transversal). BIT-11 Personas **no es bloqueante** — Tutor Responsable se crea inline en el formulario de Paciente Animal.

---
*Mantenido por Anthy. Última actualización: 29 Ago 2026 — Auditoría: roadmap BIT-13/14 corregido (Fotografías/adjuntos y Paciente Animal respectivamente), BIT-34 agregado como pendiente. BIT-11 sigue pendiente de migración por Claudy.*
