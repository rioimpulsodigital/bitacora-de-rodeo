...
- **BIT-10 (Novedades del Establecimiento) — diseño completo, en curso (9 Ago 2026).** BIT reutilizada (era "formulario de ficha clínica"). Relevamiento y spec funcional completos en Notion (Tipo D). Entidad independiente de Visita, Observación de Campo, Atención Clínica y Gasto. Registra hechos operativos del establecimiento (clima, infraestructura, movimiento, productivo, trabajo, otro). Modelo: `novedades_establecimiento` con `establecimiento_id` propio (no derivado de Visita), soft delete, trazabilidad de creación y última modificación (`created_by`, `updated_by`). Paciente Animal y Lote opcionales, seteables después de la creación. Adjuntos: tabla `novedades_adjuntos` diseñada (1..N fotos), Supabase Storage pendiente. Posición en Informe Diario propuesta (después de VISITAS, antes de PACIENTES ANIMALES) — pendiente de confirmación con Etel antes de BIT-12. **No implementar todavía — spec lista para revisión de Brenda.**
...
- **Roadmap reorganizado (BIT-32 auditoría + BIT-33 roadmap, 30 Jul 2026):** Ver BIT-33 en Notion para el plan maestro completo. BIT-10 redefinida como Novedades del Establecimiento (ver entrada anterior). Próximos módulos: BIT-10 (implementación de Novedades del Establecimiento — diseño listo), BIT-11 (Atenciones Clínicas — renombrada desde la antigua BIT-10), BIT-13/14/15 (ciclo de vida de Atención Clínica), BIT-12/16/17 (informe diario + WhatsApp). BIT-11 Personas **no es bloqueante** — Tutor Responsable se crea inline en el formulario de Paciente Animal.

---
*Mantenido por Anthy. Última actualización: 9 Ago 2026 — BIT-10 spec de diseño completa (Novedades del Establecimiento). BIT-09 cerrado el 9 Ago 2026.*
